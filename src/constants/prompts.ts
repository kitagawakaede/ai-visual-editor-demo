...
export const LINE_STAMP_DEFS: StampDef[] = [
  ...
  { text: 'りょ！', pose: '敬礼またはサムズアップのOKポーズ' },
...
const timeSlipChildBase = (scene: string) =>
  `1枚の人物写真をもとに、家族アルバムに飾る、ほのぼのとした全年齢向けの健全な記念写真を作る。これは「写真の大人の顔はそのままに、体と服装だけを変える」ユーモラスな合成写真である。【最優先・顔は写真の大人の顔をそのまま貼り付ける】写真に写っている大人の顔・頭部を、年齢も造形も大きさ感も一切変えずにそのまま使うこと。顔を幼児化・若返り・赤ちゃん顔化・子ども顔化することを固く禁止する。完成画像は『写真の大人の本人の顔のまま、首から下の体つき・服装・背景だけが変わっている』状態にすること。顔の輪郭・骨格・目鼻口の形と配置・眉・肌・表情は写真の大人と完全に同一で、顔だけ見れば誰が見ても写真の本人（大人）だと一目で分かること。丸く幼い赤ちゃん顔や、無個性で平均的なかわいい子ども顔に置き換えるのは絶対に禁止。${scene}。くり返すが、顔だけは絶対に幼くせず写真の大人のまま——変えてよいのは体つき・服装・背景・小物だけ。全身をきちんと服で覆って肌の露出をなくし、健全で微笑ましい実写風に自然に仕上げること。最終的に1枚の画像のみを生成し、Base64文字列のみを返せ。テキスト・JSON・Markdownは禁止。`

export type TimeSlipItem = {
  id: string
  label: string
  prompt: string
  // 年代別アイテムは性別で出し分ける。あればこちらを優先（テキストのみ生成）。
  promptByGender?: { man: string; woman: string }
}

// ── 旧版（参照画像あり・9年代）。新版に置き換え済みだが、いつでも戻せるよう温存。──
export const TIME_SLIP_ITEMS_LEGACY: TimeSlipItem[] = [
  {
    id: 'baby',
    label: '赤ちゃん時代',
...
// ── 新版：年代別（1950's〜2000's）────────────────────────────
// 参照画像を渡さず本人写真＋テキストのみで生成（別人の顔が混入しないので本人を保てる）。
// 各年代は性別で出し分け（撮影写真から判定）。本人の顔は保持し、服装・髪型・メイク・
// 小物・背景・色調をその年代に合わせる。
const timeSlipEraBase = (scene: string) =>
  `1枚の人物写真をもとに、その人物がその年代に生きていたら…という設定の実写風スナップ写真を作る。【最重要・本人の同一性】写真の本人の顔（顔立ち・輪郭・骨格・目鼻口の形と配置・眉・肌の質感・表情の癖）を忠実に保持し、本人だと一目で分かるようにすること。別人の顔に置き換えないこと。年齢は現在の本人のまま。${scene}。顔は本人のまま保ちつつ、服装・髪型・メイク・小物・背景・色調をその年代の雰囲気に合わせ、合成感のない当時のフィルムスナップのように自然に仕上げること。最終的に1枚の画像のみを生成し、Base64文字列のみを返せ。テキスト・JSON・Markdownは禁止。`

type EraStyle = { man: string; woman: string }
// 性別がブレないよう各プロンプト冒頭で明示する（スタイルが中性的でも男女を固定）。
const eraItem = (id: string, label: string, style: EraStyle): TimeSlipItem => ({
  id,
  label,
  prompt: timeSlipEraBase(`完成画像の人物は必ず女性として描くこと。${style.woman}`), // フォールバック
  promptByGender: {
    man: timeSlipEraBase(`完成画像の人物は必ず男性として描くこと（女性化させない）。${style.man}`),
    woman: timeSlipEraBase(`完成画像の人物は必ず女性として描くこと（男性化させない）。${style.woman}`),
  },
})

const ERA_ITEMS: TimeSlipItem[] = [
  eraItem('sixties', "1960's", {
    woman:
      '1960年代の装い。ミニスカートやAラインのミニワンピース、モッズスタイルのシャープなシルエット。タータンチェックや幾何学模様の大胆なプリント、カラフルな原色使い。目元を効かせたモッズメイク。賑やかでポップな1960年代の街を背景に',
    man:
      '1960年代の装い。アイビールック（3つボタンの紺ブレザー、ボタンダウンシャツ、コットンのチノパン、コインローファー）の「みゆき族」風、または細身のモッズスーツ。清潔感のあるきれいめスタイル。1960年代の街を背景に',
  }),
  eraItem('seventies', "1970's", {
    woman:
      '1970年代の装い。ヒッピー＆フォークロア風に、くるぶし丈のマキシワンピースや刺繍の入ったインド綿ブラウス、または膝から裾に広がるフレア（ベルボトム）パンツ。センターパート（真ん中分け）のロングヘアか軽く巻いた髪、厚底のサンダル。70年代らしいくすんだ暖色の街並みを背景に',
    man:
      '1970年代の装い。やや長めの髪に、襟の大きく広がった花柄や幾何学柄のシャツ、コーデュロイのジャケット、膝から広がるフレア（ベルボトム）パンツ。ヒッピー＆サイケ風で自由な雰囲気。70年代らしいくすんだ色調の街並みを背景に',
  }),
  eraItem('eighties', "1980's", {
    woman:
      '1980年代の装い。肩パッドの入ったパワースーツや、体のラインを強調したタイトなボディコンのワンピース、または全身を黒で統一したDCブランドのモード系。華やかでバブルな雰囲気、ボリュームのある髪と濃いめのメイク。夜の都会やディスコ風の背景に、80年代らしい色味で',
    man:
      '1980年代の装い。肩幅の広い肩パッド入りジャケットやゆったりしたダブルスーツ、テーパードパンツ、または黒基調モノトーンのDCブランド風スタイル。バブル期の華やかで都会的な雰囲気。夜の都会を背景に、80年代らしい色味で',
  }),
  eraItem('nineties', "1990's", {
    woman:
      '1990年代の装い。コギャル風に、ミニスカートにルーズソックス、厚底ブーツ、茶髪に日焼けメイク。または裏原系のビッグTシャツに古着・スニーカーのストリートスタイル。リラックス感のあるシルエット。90年代の渋谷・原宿風の街を背景に',
    man:
      '1990年代の装い。ビッグシルエットのワイド／バギーパンツに大きめのTシャツやスウェット、パーカー、スニーカー。アメカジや裏原系、ヒップホップ系のストリートスタイル。ルーズで力の抜けた雰囲気。90年代の街を背景に',
  }),
  eraItem('twothousands', "2000's", {
    woman:
      '2000年代の装い。ギャル・109系風に、ローライズのデニムやミニスカート、タイトなトップス、ロングブーツ、盛り髪に大きめのアクセサリーと派手なネイル。または上品なお姉系のタイトワンピース。華やかでギャルっぽい雰囲気。2000年代の渋谷風の背景に',
    man:
      '2000年代の装い。お兄系風に、細身のタイトなジャケットにスキニー／細身のデニム、ドレープの効いたカットソー、ブーツ、アクセサリーを多用したレイヤードスタイル、または美容師風のサロン系。細身でスタイリッシュな雰囲気。2000年代の都会を背景に',
  }),
]

// 旧版から流用する項目（既存プロンプト維持）
const legacyItem = (id: string): TimeSlipItem =>
  TIME_SLIP_ITEMS_LEGACY.find((i) => i.id === id) as TimeSlipItem

// 非行・暴走族時代も参照画像なし・テキストのみ・性別出し分けに変更。
// （ギャング参照画像＋暴力的表現が Gemini の安全ポリシーで弾かれていたため、
//  「ファッションとしての暴走族」に表現を寄せ、武器・暴力的要素は含めない）
const BOSOZOKU_ITEM = eraItem('bosozoku', '非行・暴走族時代', {
  woman:
    '昭和の女性暴走族（レディース）風の装い。刺繍の入った特攻服やスカジャンを羽織り、ボリュームのある巻いた派手な髪、濃いめのメイク。夜の街を背景に、やんちゃで強気なレトロな雰囲気。あくまでレトロなファッション・コスチュームとして表現し、武器・暴力的な要素・他者への攻撃的な行為は一切含めない',
  man:
    '昭和の暴走族（ヤンキー）風の装い。刺繍の入った特攻服やロングコート風の上着に、リーゼントやソリ込み風の髪型。夜の街やバイクの脇に立つ構図で、やんちゃで強気なレトロな雰囲気。あくまでレトロなファッション・コスチュームとして表現し、武器・暴力的な要素・他者への攻撃的な行為は一切含めない',
})

// 超優秀高校生（陰キャ）。先方仕様：現在の顔のまま・高校生の体。男女共通の装い
// （紺ブレ＋ネクタイ＋メガネ＋右手に教科書）なので性別出し分けはしない。
const MAJIME_ITEM: TimeSlipItem = {
  id: 'majime',
  label: '超優秀高校生（陰キャ）',
  prompt:
    '1枚の人物写真をもとに、その人物の高校生時代の実写風スナップ写真を作る。【最重要・本人の同一性】写真の本人の顔（顔立ち・輪郭・骨格・目鼻口の形と配置・眉・肌の質感・表情の癖）を忠実に保持し、本人だと一目で分かるようにすること。別人の顔に置き換えないこと。現在の顔のまま、体つきだけ高校生にする。【スタイル・男女共通】紺色のブレザーにネクタイの制服を着せ、黒縁メガネをかけ、右手に教科書を持たせる。勉強しか興味がなく、コミュニケーションが苦手で勉強が大好きな、できるだけ地味でおとなしい真面目な超優秀高校生の印象にすること。図書室や教室を背景に、合成感のない実写風に自然に仕上げる。最終的に1枚の画像のみを生成し、Base64文字列のみを返せ。テキスト・JSON・Markdownは禁止。',
}

// 新・本番用 9枠（1段目：赤ちゃん・小学一年生・真面目／2段目：非行・1960's・1970's／3段目：1980's・1990's・2000's）
export const TIME_SLIP_ITEMS: TimeSlipItem[] = [
  legacyItem('baby'),
  legacyItem('elementary'),
  MAJIME_ITEM,
  BOSOZOKU_ITEM,
  ...ERA_ITEMS,
]
