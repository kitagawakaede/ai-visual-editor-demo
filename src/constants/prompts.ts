...
const eraItem = (id: string, label: string, style: EraStyle): TimeSlipItem => ({
  id,
  label,
  prompt: timeSlipEraBase(style.woman), // フォールバック（通常は promptByGender を使用）
  promptByGender: { man: timeSlipEraBase(style.man), woman: timeSlipEraBase(style.woman) },
})

const ERA_ITEMS: TimeSlipItem[] = [
  eraItem('fifties', "1950's", {
    woman:
      '1950年代の装い。クリスチャン・ディオールのニュールック風に、肩のラインは自然で、ウエストを細く絞り、そこからふんわり広がるフレアの「Aライン」のワンピースかスカート。上品で清楚な雰囲気（映画「ローマの休日」のオードリー・ヘップバーン風）。柔らかくカールした短めの髪。1950年代の街並みを背景に、色あせたフィルム調の暖かい色味で',
    man:
      '1950年代の装い。前髪を長めに残した角刈り（慎太郎刈り）風の髪型に、丸首シャツやアロハシャツ、サングラス（太陽族風）。または肩幅の広い上着に裾へ細くなるパンツのボールドルックスーツ。少しやんちゃで若々しい雰囲気。1950年代の街並みを背景に、色あせたフィルム調の暖かい色味で',
  }),
...
// 新・本番用 9枠：赤ちゃん／小学一年生（既存）＋ 6年代（新規）＋ 非行・暴走族（新方式）
